...
function aiProcessor(crossPropability,mutationPropability,refferencePoints,refferenceFunction,period,setRefferenceFuntion,population,populationLength,runFlag,bestInPopulation,worseInPopulation){
	...
	function rouletteGenerationRandomizeParents(){
		...
		for(x=0; x<this.populationLength;x++){
			fuse++;
			if (fuse>100*this.populationLength){
			    alert("Population died"+preeliminatedParents.length);
		...
		var d=0;
		while(preeliminatedParents.length<this.populationLength){
		    preeliminatedParents.push(this.population[d]);
		    //console.log(this.population[d]);
		    d++;
		    
		}
		//console.log(preeliminatedParents.length,this.populationLenght);
	...
	function generateNthPopulation(){
		...
		for (x in pairs){
			...
			if (this.mutationPropability>Math.random()){
				var newLevel =Math.round(Math.random())*Math.pow(-1,Math.round(Math.random()));
				newLevel=newLevel<1?1:newLevel;
				newChromosome1[8]+=newLevel;
				
				newLevel =Math.round(Math.random())*Math.pow(-1,Math.round(Math.random()));
				newLevel=newLevel<1?1:newLevel;
				newChromosome2[8]+=newLevel;
	...
	function getResultFunctionByChromosome(chromosome){
		...
		for (var x=this.period[0];x<=this.period[1];x++){
		    //console.log(x);
		    ret.push([x,this.calculatePolynomialInPoint(chromosome[8],chromosome,x)]);
